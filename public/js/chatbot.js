  ...
  function getBotResponse(message) {
    ...
    switch (message.toLowerCase()) {
      case 'hello':
        return {
          message: 'Hi there! Would you like to confirm the exchange?',
          action: 'confirm-exchange',
        };
      case 'yes':
        if (isOrderConfirmed) {
          return {
            message: 'Your exchange has already been confirmed.',
            action: null,
          };
        } else {
          return {
            message: 'Exchange confirmed. Thank you!',
            action: 'confirm-exchange',
          };
        }
      case 'no':
      ...
      case 'negotiate':
        return {
          message: 'Sure! What is your offer for this product?',
          action: 'negotiate-offer',
        };
      case 'how can i place an exchange?':
        return {
          message:
            "To place an exchange, please select an option: You can choose to exchange through the app or in person. Please type 'in app' or 'in person' to proceed with your preferred option.",
          action: null,
        };
      ...
      case 'through app':
        mode = 'app';
        return {
          message: 'Type confirm to place the exchange through app.',
          action: null,
        };
      case 'in person':
        mode = 'direct';
        return {
          message: 'Type confirm to place exchange in person.',
          action: null,
        };
      default:
  ...
  function sendHtmlFile(filename) {
    setTimeout(() => {
      window.location.href = filename + `?id='${mode}'`;
    }, 1000);
  }
